...
const buildCatalogView = ({ category, page = 0 }) => {
  ...
  embed.setFooter({ text: `CAT:${category || 'ALL'}|PAGE:${page}` });
...
const parseCatalogState = (embed) => {
  ...
  const [catPart, pagePart] = footer.split('|');
  const category = catPart?.replace('CAT:', '') || 'ALL';
  const page = Number(pagePart?.replace('PAGE:', '')) || 0;
  return { category: category === 'ALL' ? undefined : category, page };
};
