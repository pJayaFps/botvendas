...
const listProducts = (botId = 1, category) => {
  if (category) {
    return db.prepare('SELECT * FROM products WHERE active = 1 AND bot_id = ? AND category = ? ORDER BY id DESC').all(botId, category);
  }
  return db.prepare('SELECT * FROM products WHERE active = 1 AND bot_id = ? ORDER BY id DESC').all(botId);
};
